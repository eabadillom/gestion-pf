...
public class EntradaBL {
	...
	public static synchronized String crearFolio(ConstanciaDeDeposito constancia, Planta planta)
		...
		serie = serieDAO
				.buscarPorClienteTipoSerieAndPlanta(constancia.getCteCve().getCteCve(), "I", planta.getPlantaCve())
				.orElseThrow(
						() -> new InventarioException("No es posible generar el folio de la constancia de depósito."));
	...
	public static synchronized UnidadDeProducto buscarUnidadProducto(UnidadDeProducto unidad)
		...
		udp = udpDAO.buscarPorProductoUnidad(unidad.getProductoCve().getProductoCve(),
				unidad.getUnidadDeManejoCve().getUnidadDeManejoCve());

		if (udp == null) {
			log.info(
					"La unidad de producto indicada no existe, se registrará una nueva: id producto = {}, id unidad = {}",
	...
	public static synchronized void agregarPartidaCompleta(ConstanciaDeDeposito constancia, BigDecimal numTarimas,
			Partida partida)
	...
	public static synchronized List<Tarima> crearTarimas(ConstanciaDeDeposito constancia, Integer numeroTarimas,
			Partida partida, List<Tarima> tarimas)
			throws InventarioException, CloneNotSupportedException {
		Tarima t = null;
		Partida p = null;
		UnidadDeProductoDAO udpDAO = null;
	...
	public static synchronized void agregarServicio(ConstanciaDeDeposito constancia, PrecioServicio ps,
			BigDecimal cantidad)
			throws InventarioException {
	...
	public static synchronized void agregarServicio(ConstanciaDeDeposito constancia, Servicio servicio,
			BigDecimal cantidad)
			throws InventarioException {
	...
	public static synchronized void eliminarServicio(ConstanciaDeDeposito constancia,
			ConstanciaDepositoDetalle servicio)
			throws InventarioException {
		if (constancia == null)
			throw new InventarioException("Debe proporcionar una constancia de depósito.");
	...
	public static String obtnerDescripcionDatosAdicionalesBitacora(ConstanciaDeDeposito entrada, String propiedad) {

		String descripcionEvento = "Modificó el dato adicional " + propiedad
				+ " a: ";

		switch (propiedad) {
			case "temperatura":
				descripcionEvento = descripcionEvento + entrada.getTemperatura();
				break;

			case "nombre transportista":
				descripcionEvento = descripcionEvento + entrada.getNombreTransportista();
				break;

			case "placas transporte":
				descripcionEvento = descripcionEvento + entrada.getPlacasTransporte();
				break;

			case "observaciones":
				descripcionEvento = descripcionEvento + entrada.getObservaciones();
				break;

			default:
				throw new ValidationException("El dato de dato adicional, no existe");
		}

		return descripcionEvento;
	}

	public static String obtenerDescripcionCargaPorPartidaBitacora(Partida partida, String propiedad) {
		String descripcionEvento = "Modificó ";

		switch (propiedad) {
			case "cantidad":
				descripcionEvento = descripcionEvento + " la cantidad del producto "
						+ partida.getUnidadDeProductoCve().getProductoCve().getProductoDs() + " a: "
						+ partida.getCantidadTotal() + " "
						+ partida.getUnidadDeProductoCve().getUnidadDeManejoCve().getUnidadDeManejoDs();
				break;

			case "peso":
				descripcionEvento = descripcionEvento + " el peso del producto "
						+ partida.getUnidadDeProductoCve().getProductoCve().getProductoDs() + " a: "
						+ partida.getPesoTotal() + " Kg";
				break;

			default:
				throw new ValidationException("El dato de carga por partida, no existe");
		}

		descripcionEvento = descripcionEvento + ", de la tarima: " + partida.getTarima().getNombre();

		return descripcionEvento;
	}

	public static String obtenerDescipcionServiciosBitacora(String propiedad,
			boolean estadoServicio) {
		String descripcionEvento = "Cambió el estado del servicio ";
		String estado = (estadoServicio) ? "seleccionado" : "deseleccionado";

		descripcionEvento = descripcionEvento + " " + propiedad + " a: " + estado;

...
public class EmisoresCFDISDAO extends IBaseDAO<EmisoresCFDIS, Integer> {
	...
	public EmisoresCFDIS buscarPorId(Integer cd_emisor) {
		...
		try {
			...
			em = EntityManagerUtil.getEntityManager();
			emi = em.createNamedQuery("EmisoresCFDIS.findBycdEmisor", EmisoresCFDIS.class)
					.setParameter("cd_emisor", cd_emisor).getSingleResult();
	...
	public List<EmisoresCFDIS> buscarPorCriterios(EmisoresCFDIS e) {
		...
		try {
			...
			listaEmisores = em.createNamedQuery("EmisoresCFDIS.findByregimenFiscal", EmisoresCFDIS.class)
					.setParameter("cd_regimen", e.getCd_regimen()).getResultList();

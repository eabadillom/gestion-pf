...
public class RepServiciosDAO extends IBaseDAO<RepServicios, Integer> {
	...
	public List<RepServicios> buscar(Date fechaIni, Date fechaFin, Integer idCliente) {
		...
		try {
			sql = "SELECT " + "	cs.folio, " + "	cs.folio_cliente, " + "	cs.fecha, " + "	cs.observaciones, "
					+ "	c.numero_cte, " + "	c.cte_nombre, " + "	prd.producto_ds, " + "	ps.cantidad_de_cobro, "
					+ "	ps.cantidad_total, " + "	COALESCE(udm.unidad_de_manejo_ds, 'Piezas') unidadManejo, "
					+ "	upper(s.servicio_ds) Servicio, " + "	tc.nombre as tipo_cobro, " + "	csd.servicio_cantidad "
					+ "FROM constancia_de_servicio cs " + "INNER JOIN cliente c ON c.cte_cve = cs.cte_cve "
					...
					+ "INNER JOIN tipo_cobro tc ON tc.id = s.cobro " + "WHERE "
					+ "	(cs.fecha BETWEEN :fechaIni AND :fechaFin) "
					+ "	AND (c.cte_cve = :idCliente OR :idCliente IS NULL) " + "	AND (cs.status <> 4) " + "ORDER BY "
					+ "	c.cte_nombre ASC, " + "	cs.folio_cliente ASC ";

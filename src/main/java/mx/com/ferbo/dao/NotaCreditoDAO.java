	...
	public List<NotaCredito> buscarPorCriterios(Date fechaInico, Date fechaFin, int idCliente) {
		...
		try {
			...
			if (fechaInico != null && fechaFin != null) {// si ambos parametros son diferente de null entra
				whereCause.add("(n.fecha BETWEEN :fechaInicio AND :fechaFinal)");// añade a la lista de String que
																					// representa los where
				paramaterMap.put("fechaInicio", fechaInico);// seteamos los parametros*
				paramaterMap.put("fechaFinal", fechaFin);// *
			}
			/*
			 * if (folioCliente != null && !"".equalsIgnoreCase(folioCliente.trim())) {//si
			 * el folio es diferente de null y diferente de una cadena "" entra
			 * whereCause.add("n.folioCliente = :folioCliente");//comparamos folioCliente de
			 * constanciadeS de lo que se setea en :folioCliente
			 * paramaterMap.put("folioCliente", folioCliente);//seteamos folioCliente }
			 */
			if (idCliente != 0) {// si el id es diferente de 0 entra
				// cliente.setCteCve(idCliente);//al objeto instanciado le seteamos el id del
				// parametro idCliente
				whereCause.add("n.idcliente = :idCliente");// comparamos el cliente de constanciadeservicio con lo que
															// se seteo al :idCliente
			...
			@SuppressWarnings("unchecked")
			List<NotaCredito> listado = (List<NotaCredito>) q.getResultList();// creamos tipo de lista que se retornara
																				// y casteamos el resultado de la query

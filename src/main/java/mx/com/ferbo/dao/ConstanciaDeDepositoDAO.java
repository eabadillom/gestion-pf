...
public class ConstanciaDeDepositoDAO extends IBaseDAO<ConstanciaDeDeposito, Integer> {
	...
	public List<ConstanciaDeDeposito> buscarTodos() {
		...
		try {
			...
			return em.createNamedQuery("ConstanciaDeDeposito.findAll", ConstanciaDeDeposito.class).getResultList();
		} catch (Exception e) {
	...
	public List<ConstanciaDeDeposito> buscarPorCliente(Integer cteCve) {
		...
		try {
			em = EntityManagerUtil.getEntityManager();
			listado = em.createNamedQuery("ConstanciaDeDeposito.findByCteCve", ConstanciaDeDeposito.class)
	...
	public List<ConstanciaDeDeposito> buscarPorCriterios(String folioCliente, Date fechaInico, Date fechaFin,
			int idCliente) {
		...
		try {
			...
			if (fechaInico != null && fechaFin != null) {// si ambos parametros son diferente de null entra
				whereCause.add("(cdd.fechaIngreso BETWEEN :fechaInicio AND :fechaFinal)");// añade a la lista de String
																							// que representa los where
				paramaterMap.put("fechaInicio", fechaInico);// seteamos los parametros*
				paramaterMap.put("fechaFinal", fechaFin);// *
			}
			if (folioCliente != null && !"".equalsIgnoreCase(folioCliente.trim())) {// si el folio es diferente de null
																					// y diferente de una cadena ""
																					// entra
				whereCause.add("cdd.folioCliente = :folioCliente");// comparamos folioCliente de constanciadeS de lo que
																	// se setea en :folioCliente
			...
			if (idCliente != 0) {// si el id es diferente de 0 entra
				...
				whereCause.add("cdd.cteCve = :idCliente");// comparamos el cliente de constanciadeservicio con lo que se
															// seteo al :idCliente
				paramaterMap.put("idCliente", cliente);// seteamos idCliente el objeto cliente instanciado
			}

			queryBuilder.append(" WHERE " + StringUtils.join(whereCause, " AND "));// al query le añadimos los
																					// wherecause dependiendo en que
																					// condiciones entre

			q = em.createQuery(queryBuilder.toString());// a la sesion le pasamos el query creado que se encuentra en
														// queryBuilder y se lo pasamos como String

			for (String key : paramaterMap.keySet()) {// recorremos las llaves que contiene parameterMap
				q.setParameter(key, paramaterMap.get(key));// por cada key le seteamos a q (query) los parametros con la
															// llave (key=string) y con .get el objeto de la key
			}

			List<ConstanciaDeDeposito> listado = (List<ConstanciaDeDeposito>) q.getResultList();// creamos tipo de lista
																								// que se retornara y
																								// casteamos el
	...
	@SuppressWarnings("unchecked")
	public List<ConstanciaDeDeposito> buscarPor(String folioCliente, Integer idCliente, Date fechaInicio,
			Date fechaFin) {
		EntityManager em = null;
		List<ConstanciaDeDeposito> listado = null;
		...
		try {
			...
			listado = em.createNativeQuery("SELECT * FROM (\n" + "	SELECT * FROM (\n"
					...
					ConstanciaDeDeposito.class).setParameter("idCliente", idCliente)
					.setParameter("folioCliente", folioCliente).setParameter("fechaInicio", fechaInicio)
	...
	public ConstanciaDeDeposito buscarPorFolioCliente(String folioCliente, boolean isFullInfo) {
		...
		try {
			...
			constancia = em.createNamedQuery("ConstanciaDeDeposito.findByFolioCliente", ConstanciaDeDeposito.class)
					.setParameter("folioCliente", folioCliente).getSingleResult();
			...
			List<ConstanciaDepositoDetalle> constanciaDepositoDetalleList = constancia
					.getConstanciaDepositoDetalleList();
			...
			for (Partida partida : partidaList) {
				...
				for (DetallePartida dp : detallePartidaList) {
					log.info("Detalle Partida: {} - {}", dp.getDetallePartidaPK().getPartidaCve(),
							dp.getDetallePartidaPK().getDetPartCve());
				}
			}
	...
	public ConstanciaDeDeposito buscarPorFolioCliente(String folio) {
		...
		try {
			...
			constancia = em.createNamedQuery("ConstanciaDeDeposito.findByFolioCliente", ConstanciaDeDeposito.class)
					.setParameter("folioCliente", folio).getSingleResult();
	...
	public boolean tieneSalidas(Integer folio) {
		...
		try {
			sql = "select cdd.FOLIO, dcs.ID " + "from constancia_de_deposito cdd "
					+ "inner join PARTIDA p ON cdd.FOLIO = p.FOLIO "
					+ "left outer join detalle_constancia_salida dcs ON dcs.PARTIDA_CVE = p.PARTIDA_CVE "
					+ "where cdd.FOLIO = :folio " + "GROUP BY cdd.FOLIO, dcs.ID ";

			em = EntityManagerUtil.getEntityManager();
			results = em.createNativeQuery(sql).setParameter("folio", folio).getResultList();
	...
	public boolean tieneFacturas(Integer folio) {
		...
		try {
			sql = "select cdd.FOLIO, cf.id " + "from constancia_de_deposito cdd "
					+ "left outer join constancia_factura cf ON cf.folio = cdd.FOLIO " + "where cdd.FOLIO = :folio "

...
public class FacturacionServiciosDAO extends IBaseDAO<ConstanciaFacturaDs, Integer> {
	...
	public List<ConstanciaFacturaDs> buscarNoFacturados(Integer idCliente) {
		...
		try {
			...
			list = new ArrayList<>();
			sql = "SELECT " + " cs.FOLIO, " + "	cs.CTE_CVE, " + "	cs.FECHA, " + "	cs.NOMBRE_TRANSPORTISTA, "
					+ "	cs.PLACAS_TRANSPORTE, " + "	cs.OBSERVACIONES, " + "	cs.FOLIO_CLIENTE, "
					+ "	cs.VALOR_DECLARADO, " + "	cs.STATUS " + "FROM constancia_de_servicio cs "
					+ "INNER JOIN cliente cte ON cs.CTE_CVE = cte.CTE_CVE " + "LEFT OUTER JOIN ( "
			...
			for (ConstanciaDeServicio constancia : listaConstancias) {
				List<ConstanciaServicioDetalle> allConstanciaServicioDetalle = constancia
						.getConstanciaServicioDetalleList();// recuperando constancias de servicio detalle de servicio
															// ds
				...
				lConstanciaFactura = listaTmpConstancias.stream().filter(c -> (c.getFactura().getStatus().getId() == 1 // Status
																														// por
																														// cobrar
				...
				// FALTA RELACION DE CONSTANCIA FACTURA DS CON LA CONSTANCIA DE SERVICIO
				// constancia.setConstanciaFacturaDsList(null);
				list.add(cf);

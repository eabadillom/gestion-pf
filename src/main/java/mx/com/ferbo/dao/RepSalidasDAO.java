...
public class RepSalidasDAO extends IBaseDAO<RepSalidas, Integer> {
	...
	public List<RepSalidas> buscar(Date fechaIni, Date fechaFin, Integer idCliente, Integer idPlanta,
			Integer idCamara) {
		...
		try {
			sql = "SELECT " + "	cs.fecha, " + "	cdd.folio_cliente, " + "	cs.numero, " + "	dcs.producto, "
					+ "	dcs.unidad, " + "	dcs.cantidad, " + "	dcs.peso, " + "	c.numero_cte, " + "	c.cte_nombre, "
					+ "	cdd.fecha_ingreso, " + "	p.partida_cve, " + "	p.cantidad_total, " + "	p.peso_total, "
					+ "	cam.camara_cve, " + "	cam.camara_abrev AS camara, " + "	plt.planta_cve, "
					+ "	plt.planta_abrev AS planta " + "FROM constancia_salida cs "
					...
					+ "INNER JOIN planta plt ON plt.planta_cve = cam.planta_cve " + "WHERE cs.status NOT IN (2) "
					...
					+ "	AND (cam.camara_cve = :idCamara OR :idCamara IS NULL) " + "ORDER BY " + "	c.cte_nombre ASC, "
					+ "	p.PARTIDA_CVE ASC, " + "	cdd.folio_cliente ASC, " + "	cs.fecha ASC ";

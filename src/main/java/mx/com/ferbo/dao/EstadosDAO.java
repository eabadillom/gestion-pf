...
public class EstadosDAO extends IBaseDAO<Estados, Integer> {
	...
	public Estados buscarPorId(Integer idEstado) {
		...
		try {
			...
			sql = entity.createNamedQuery("Estados.findByEstadoCve", Estados.class).setParameter("estadoCve", idEstado);
	...
	public List<Estados> buscarPorCriteriosEstados(Estados e) {
		...
		listado = em.createNamedQuery("Estados.findByPaisCve", Estados.class)
				.setParameter("paisCve", e.getEstadosPK().getPaisCve()).getResultList();
		return listado;
	}
	...
	public String eliminar(Estados estados) {
		try {
			...
			em.getTransaction().begin();
			em.createQuery(
					"DELETE FROM Estados e WHERE e.estadosPK.paisCve =:paisCve and e.estadosPK.estadoCve =:estadoCve")
	...
	public List<Estados> buscaPorId(Integer id) {
		EntityManager em = EntityManagerUtil.getEntityManager();
		return em.createNamedQuery("Estados.findByEstadoCve", Estados.class).setParameter("estadoCve", id)
				.getResultList();
	}
	...
	public List<Estados> buscarPorPais(Paises pais) {
		...
		try {
			...
			list = em.createNamedQuery("Estados.findByPaisCve", Estados.class)
					.setParameter("paisCve", pais.getPaisCve()).getResultList();
		} catch (Exception ex) {

	...
	public FacturaMedioPago buscarPorFactura(Integer facturaId) throws SQLException {
		...
		try {
			...
			query = em.createNativeQuery(
					"SELECT fmp.factura_id, fmp.fmp_id, fmp.mp_id, fmp.mp_descripcion, fmp.fmp_porcentaje, fmp.fmp_referencia, fmp.fmp_forma_pago "
							+ "FROM factura_medio_pago fmp WHERE fmp.factura_id = :facturaId ",
					FacturaMedioPago.class).setParameter("facturaId", facturaId);
	...
	public String actualizar(FacturaMedioPago e) {
		...
		try {
			...
			em.createNativeQuery(
					"UPDATE factura_medio_pago fmp SET fmp.mp_id =:mpId, fmp.mp_descripcion = :mpDescripcion WHERE fmp.factura_id = :facturaId",
					FacturaMedioPago.class).setParameter("mpId", e.getMpId())

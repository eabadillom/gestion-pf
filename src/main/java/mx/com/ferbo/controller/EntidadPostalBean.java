...
public class EntidadPostalBean implements Serializable {
	...
	public void eliminandoEntidadPostal() {
		...
		List<AsentamientoHumano> listaTmpAsentamientoH = listaAsentamientoHumano.stream().filter(
				ah -> ah.getAsentamientoHumanoPK().getEntidadpostalCve() == entidadPostalSelect.getEntidadpostalCve())
		...
		if (listaTmpAsentamientoH.isEmpty()) {
			...
			if (entidadPostalDAO.eliminar(entidadPostalSelect) == null) {
				...
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Error", "Ocurrió un error al intentar eliminar el Tipo de Asentamiento"));
			}
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR,
					"Existe relacion Entidad Postal - AsentamientoHumano",

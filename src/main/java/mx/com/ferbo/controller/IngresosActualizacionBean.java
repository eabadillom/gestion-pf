...
public class IngresosActualizacionBean implements Serializable {
	...
	public void deletePago() {
		...
		try {
			...
			if (respuesta != null) {
				log.info("Problema al actualizar el status de la factura {}-{}: {}", factura.getNomSerie(),
						factura.getNumero(), respuesta);
				throw new InventarioException("Ocurrió un problema para actualizar el status de la factura "
						+ factura.getNomSerie() + " - " + factura.getNumero());
			}

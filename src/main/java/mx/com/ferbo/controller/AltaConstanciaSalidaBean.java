...
public class AltaConstanciaSalidaBean implements Serializable {
	...
	private void validaSaldo() throws InventarioException {
		...
		if (saldoVencido.compareTo(BigDecimal.ZERO) > 0) {
			...
			log.info("El cliente {} presenta un saldo vencido: {}", this.clienteSelect.getCteNombre(),
					this.saldo.getSaldo());
		}
...
//			throw new InventarioException("El cliente no tiene permitida la salida de mercancía. Por favor, comuníquese con el área de Facturación.");
		...
		if (isHabilitarSalida && candadoSalida.getNumSalidas() > 0) {
			log.info("El cliente {} presenta un saldo vencido ({}), pero tiene permitidas las salidas",
					this.clienteSelect.getCteNombre(), this.saldo.getSaldo());
			return;
		}
	...
	public void deleteDetalleConstanciaSalida(DetalleConstanciaSalida detalle) {
		...
		try {
			...
			for (Inventario i : listaInventarioCopy) {
				if ((i.getProducto().getProductoDs().equals(detalleSalida.getProducto()))
						&& (i.getFolioCliente().equals(detalleSalida.getFolioEntrada()))) {
					inventarioTemp = i;
					break;
				}
			}

...
public class EmisoresCFDISBean implements Serializable {
	...
	public void guardarCertificado() {
		 ...
		 * FacesMessage(FacesMessage.SEVERITY_ERROR, "Certificado .cer o .key",
		 * "Ambos archivos deben estar cargados"));
		 * PrimeFaces.current().ajax().update("form:messages"); }else {
		 */
		...
		try {
			...
			List<CsdRsp> collectedCsds = certificados.stream()
					.filter(c -> c.getRfc().equals(certificado.getEmisor().getNb_rfc())).collect(Collectors.toList());
		...
		if (certi == null) {
			...
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Certificado agregado con exito" + certificado.getCdCertificado(), null));
			PrimeFaces.current().ajax().update("form:messages", "form:dt-emisor");
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Error, verificar datos" + certificado.getCdCertificado(), certi));
	...
	public void actualizarEmisor() {
		...
		if (validacion(tipoP, rfcValidacion) == false) {
			FacesMessage message = new FacesMessage(FacesMessage.SEVERITY_INFO, "Message",
					" El RFC debe tener almenos 13 caracteres");
			PrimeFaces.current().dialog().showMessageDynamic(message);
			return;
		} else {
			if (tipoPersona == "M" || regimenCapital == "") {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_INFO, "Favor de ingresar el regimen capital", null));
				PrimeFaces.current().ajax().update("form:messages");
				return;
			}
			...
			if (em == null) {
				...
				listaEmisor = emisoresDAO.findall(true);
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
						"Emisor modificado con exito" + emisor.getCd_emisor(), null));
				PrimeFaces.current().ajax().update("form:messages", "form:dt-emisor");
			} else {
				FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
						"Error al modificar emisor" + emisor.getCd_emisor(), em));
				PrimeFaces.current().ajax().update("form:messages");
			}
		}
		this.emisor = new EmisoresCFDIS();
	...
	public void guardarEmisor() {
		...
		if (em == null) {
			...
			listaEmisor = emisoresDAO.findall(true);
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Emisor agregado con exito" + emisor.getCd_emisor(), null));
			PrimeFaces.current().ajax().update("form:messages", "form:dt-emisor");
		} else {
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO,
					"Error al agregar el emisor" + emisor.getCd_emisor(), em));
			PrimeFaces.current().ajax().update("form:messages");
		}
	...
	public void RegimenSelect() {
		...
		if (validacion(tipoPersona, rfc) == false) {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_ERROR, "El RFC es incorrecto", null));
			PrimeFaces.current().ajax().update("form:messages");
		} else {
			FacesContext.getCurrentInstance().addMessage(null,
					new FacesMessage(FacesMessage.SEVERITY_INFO, "El RFC es correcto", null));
			PrimeFaces.current().ajax().update("form:messages");
		}
